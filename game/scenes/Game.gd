...
onready var multiplyer_label = $UI/Multiplyer
...
# Post jam release idea: Turn level json files into images that'll be read.
...
onready var bread_bop_timer = $BreadBop
...
var multiplyer : float = 0.0
var toast_multiplyer : int = 1
# MUSIC TIMING JUNK
var time_begin
var time_delay
...
func _physics_process(delta):
	...
	multiplyer_label.text = "MULTIPLIER: " + String(multiplyer) + "X"
	if not ScoreTracker.cheats[1]["value"]:
...
func consecutive_combos():
	multiplyer = (max(1, (consecutive_hits / 2) + (consecutive_sweets * 1.5) * toast_multiplyer))
...
func bake_bread():
	if pattern.size() > current_beat: 
		timer.wait_time *= pattern[current_beat]["beat_time"]
		for oven in pattern[current_beat]["spawner"]:
			ovens[oven-1].spawn_bread(bread_speed)
		current_beat += 1
	else:
		bread_bop_timer.stop()
		timer.stop()
		yield(get_tree().create_timer(timer.wait_time+3.0), "timeout")
		var menu = MENU.instance()
		menu.can_resume = false
		if not ScoreTracker.cheats[0]["value"]:
			if not ScoreTracker.cheats[1]["value"]:
				ScoreTracker.set_scores(points, song_data["SONG_INFO"]["Name"])
				ScoreTracker.set_completion(song_data["SONG_INFO"]["Name"], "Beaten", true)
		if ScoreTracker.cheats[2]["value"]:
			if not ScoreTracker.get_completion(song_data["SONG_INFO"]["Name"])["Low Vis"]:
				ScoreTracker.set_completion(song_data["SONG_INFO"]["Name"], "Low Vis", true)
		if ScoreTracker.cheats[3]["value"]:
			if not ScoreTracker.get_completion(song_data["SONG_INFO"]["Name"])["Sudden Death"]:
				ScoreTracker.set_completion(song_data["SONG_INFO"]["Name"], "Sudden Death", true)
				if ScoreTracker.cheats[3]["sub-cheat"]["value"]:
					if not ScoreTracker.get_completion(song_data["SONG_INFO"]["Name"])["Perfect"]:
						ScoreTracker.set_completion(song_data["SONG_INFO"]["Name"], "Perfect", true)
						ScoreTracker.set_completion(song_data["SONG_INFO"]["Name"], "Sudden Death", true)
		add_child(menu, true)
...
func game_over():
	...
	tween.interpolate_property(self, "toast_multiplyer", toast_multiplyer, 0, 1.0, Tween.TRANS_LINEAR, Tween.EASE_OUT)
	tween.interpolate_property(self, "multiplyer", multiplyer, 0, 1.0, Tween.TRANS_LINEAR, Tween.EASE_OUT)
...
func toasting(multiplyer):
	toast_multiplyer += multiplyer
...
func _ready():
	...
	bake_interval = 60.0 / song_data["SONG_INFO"]["BPM"]
	bread_speed = song_data["SONG_INFO"]["speed"]
	# Descent = 2 bps
